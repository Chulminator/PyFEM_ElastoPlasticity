...
def DataNormlize(model, inp, grad, tangent):
    ...
    print("input (ev, es): 2.0, 3.0")
    print("scaled_input = inp*scales_inp+limits_inp")
    print("inp1 = 2.0*",scales_inp[0],"+",limits_inp[0])
    print("inp2 = 3.0*",scales_inp[1],"+",limits_inp[1])
...
def Training(model, E, nu):
    ...
    ModelTraining(model, 0.0005, 3500, X_train, y_train, X_test, y_test)
...
def PlotTrainingResult(model, E, nu):
    ...
    ax.set_title('p true', fontsize=11)
    plt.xlabel('ev')
    plt.ylabel('es')
    plt.rc('font',size=11)
    plt.gcf().subplots_adjust(bottom=0.20)
    ...
    ax.set_title('p pred', fontsize=11)
    fig.colorbar(c, ax=ax)
    ...
    ax.set_title('q true', fontsize=11)
    plt.xlabel('ev')
    plt.ylabel('es')
    plt.rc('font',size=11)
    fig.colorbar(c, ax=ax)
    ...
    ax.set_title('q pred', fontsize=11)
    fig.colorbar(c, ax=ax)
    plt.xlabel('ev')
    plt.ylabel('es')
    plt.rc('font',size=11)
