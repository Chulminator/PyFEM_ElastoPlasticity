...
def ConstructStiffness(Fem, Node, Element):
    ...
    for ind1, Edof in enumerate(Element.Connectivity):
        ...
        for ind, gp in enumerate(Fem.GP):
            ...
            #if not(GPdata.eps_e[0] == 0.):
                #print(Node.du[ENdof])
                #print(B_GP @ Node.du[ENdof])
                #print(GPdata.eps_e)
                #print(GPdata.eps_p)
                #print(GPdata.deps)
                #print(GPdata.stress)
                #print(GPdata.D @ B_GP @ Node.du[ENdof])
                ##print(GPdata.lamda)
                ##print(GPdata.D)
                #print("=====stop=====\n\n")
                #input("stop\n\n")
...
def Solve(Node, Global_K):
    ...
    F_out[IndexBCN] = BCN
    F_total = F_out[IndexBCN] - Node.F_int[IndexBCN] + Node.F_int0[IndexBCN]
