...
class ConstitutiveLaw():
    ...
    def Voigt2Tensor(self, voigt, flag='strain'):
        ...
        Tensor = np.zeros((self.Dim,self.Dim))
    ...
    def ReturnMapping(self, GPstate):
        if self.Dim == 2:
            ...
            if (self.ReturnMappingMode == 'eigenspace'):
                ...
                self.ReturnMappingTensorSpace3D(GPstate)
                #print("not prepared yet")
            elif(self.ReturnMappingMode == 'pqspace' ):
                assert False, "pqspace not prepared yet"
            #print("pqspace not prepared yet")
            #exit(1)
        #print(GPstate.sigma)
        #print(GPstate.D)
        #exit(1)
    ...
    def ReturnMappingEigenSpace2D(self, GPstate):
        # http://mmc.rmee.upc.edu/documents/Slides/MASTER/MultimediaCourse_Chapter07_v2S.pdf
        ...
        sigma_tr = sigma_tr1*np.tensordot(n1,n1,axes=0) + sigma_tr2*np.tensordot(n2,n2,axes=0)
        ...
        if YF <= 0.:
            ...
            assert False, "Plasticity is not implemented yet"
            #print(">> Plastic!")
    ...
    def ReturnMappingTensorSpace3D(self, GPstate):
