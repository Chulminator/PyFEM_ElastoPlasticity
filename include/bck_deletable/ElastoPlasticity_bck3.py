...
def ElementSetUp(Fem, Node, Element):
    ...
    for Edof in Element.Connectivity:
        ...
        for gp in Fem.GP:
            ...
            Init_tensor2.append(np.copy(tmp_tensor1))
        Element.B_matrix.append(B_matrix)
        Element.Jacc.append(Jacc_Elem)
        Element.Area.append(Area)

        Element.GPstrain_e.append(Init_tensor.copy())
        Element.GPstrain_p.append(Init_tensor3.copy())
        Element.GPlamda.append(Init_tensor1.copy())
        Element.GPstress.append(Init_tensor2.copy())
...
def ConstructStiffness(Fem, Node, Element, ConstitutiveModel):
    ...
    for ind1, Edof in enumerate(Element.Connectivity):
        ...
        for ind, gp in enumerate(Fem.GP):
            ...
            Jacc_GP = Jacc[ind]
            GPdata  =  GPstate()
            GPdata.eps_e  = np.copy(GPeps_e[ind])
            GPdata.eps_p  = np.copy(GPeps_p[ind])
            GPdata.lamda  = np.copy(GPlamda[ind])
            GPdata.deps   = B_GP @ Node.du[ENdof]
            GPdata.stress = np.copy(GPstress[ind])

            ConstitutiveModel.ReturnMapping(GPdata)

            #print("GPstress[ind]\t", GPstress[ind])
            ##print("GPdata.stress\t", GPdata.stress)
            #print("GPdata.dstress\t", GPdata.dstress)
            #print("-"*50)
            #GPstress[ind] += GPdata.dstress  # not working
            #GPeps_e[ind]  += GPdata.deps_e   # not working
            #GPeps_p[ind]  += GPdata.deps_p   # not working
            #GPstress[ind]  = GPdata.stress
            GPeps_e[ind]   = GPdata.eps_e
            GPeps_p[ind]   = GPdata.eps_p

            GPdata.lamda   = GPdata.lamda
            ...
            if Fem.Dimension == 2:
                ...
                GPF_int += B_GP.T @ GPdata.stress * Jacc_GP * gp[-1]
            else:
        ...
        #input("")
        GPstress_tmp.append(ElemStress)
        Element.Stiffness.append(E_K)
        Node.F_int[ENdof] += GPF_int
    Element.GPstress = GPstress_tmp
    return
...
def Solve(Node, Global_K):
    ...
    #print(np.linalg.norm(F_total)**2/(1.+np.linalg.norm(Node.F_ext[IndexBCN])**2))
    #input("====================================================================")
...
class ConstitutiveLaw():
    ...
    def ReturnMappingEigenSpace(self, GPstate):
        ...
        #print("\nsigma_tr_principal_mag\t",sigma_tr_principal_mag)
        #print("\nn1\t",n1)
        #print("\nn2\t",n2)
        #print("\nn3\t",n3)
