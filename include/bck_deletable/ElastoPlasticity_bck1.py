...
class ConstitutiveLaw():
    ...
    def ReturnMappingEigenSpace(self, GPstate):
        ...
        sigma_tr = np.tensordot(D, deps) + sigma_init

        sigma_tr_principal_mag, sigma_tr_principal_vec = np.linalg.eigh(sigma_tr)

        n1 = sigma_tr_principal_vec[:,0]
        n2 = sigma_tr_principal_vec[:,1]
        n3 = sigma_tr_principal_vec[:,2]
        ...
        if YF <= 0.:
            ...
            #tmp = np.tensordot(D, eps_e_init+deps)

            HSP = self.InitialaPressure()
            print("\nHSP\n",HSP)

            eps_HSP_principal_mag = np.linalg.solve(Ce_principal, np.diag(HSP))
            print("\neps_HSP_principal_mag\n",eps_HSP_principal_mag)

            #print("\nsigma tr\n",tmp)
            print("\nsigma tr\n",sigma_tr)
            print(sigma_tr_principal_mag)
            ##eps_e_tr_principal_mag = np.linalg.solve(Ce_principal, sigma_tr_principal_mag- initial_stress)
            eps_e_tr_principal_mag = np.linalg.solve(Ce_principal, sigma_tr_principal_mag)
            print("\neps_e_tr_principal_mag\n",eps_e_tr_principal_mag)
            tmp1, _ = np.linalg.eig(eps_e_init)
            tmp2, _ = np.linalg.eig(eps_e_init+deps)
            print(tmp1)
            print(tmp2)
            print(tmp2+eps_HSP_principal_mag)
            input("")

            #eps_e_tr_principal_mag = np.linalg.solve(Ce_principal, sigma_tr_principal_mag)

            eps_e_tr_principal_mag, _ = np.linalg.eig(eps_e_init+deps)
            eps_e_tr1 = eps_e_tr_principal_mag[0]
            eps_e_tr2 = eps_e_tr_principal_mag[1]
            eps_e_tr3 = eps_e_tr_principal_mag[2]

            #sigma_principal_mag, _ = np.linalg.eigh(sigma_init)
            #eps_e_principal_mag    = np.linalg.solve(Ce_principal, sigma_principal_mag)
            eps_e_tr_principal_mag, _ = np.linalg.eig(eps_e_init+deps)
            ...
            for ii in range(20):
                ...
                # Update derivatives
                # >> First order derivatives
                dfdsig1, dfdsig2, dfdsig3, dfdlamda = self.PlasticityModel.df(sigma1_current,\
                                                                              sigma2_current,\
                                                                              sigma3_current,\
                                                                              lamda_current)
            ...
            eps_e = eps_e_init + deps_e
            eps_p = eps_p_init + deps_p
            lamda = lamda + x[3]
