...
class ConstitutiveLaw():
    ...
    def ReturnMappingEigenSpacePS(self, GPstate):
        ...
        dsigxdepsex = lam + 2. *mu
        dsigxdepsey = lam
        dsigydepsex = lam
        dsigydepsey = lam + 2. *mu
        #dsigxydepsexy
        ...
        if YF <= 0.:
            ...
            eps_ex = eps_e_init[0]
            eps_ey = eps_e_init[1]
            eps_exy = eps_e_init[2]

            eps_e_trx = eps_e_tr[0]
            eps_e_try = eps_e_tr[1]
            eps_e_trxy = eps_e_tr[2]
            ...
            for ii in range(20):
                ...
                # Update derivatives
                # >> First order derivatives
                dfdsigx, dfdsigy, dfdsigxy, dfdlamda = self.PlasticityModel.df(sigma_current[0], sigma_current[1], sigma_current[2], lamda_current)

                # >> Second order derivatives
                #d2fdsig1dsig1, d2fdsig2dsig2, _, d2fdsig1dsig2, _, _\
                    #= self.PlasticityModel.df2(sigma_current[0], sigma2_current, 0.)

                # Update residual
                res[0] = x[0] - eps_e_trx  + x[3]*dfdsigx
                res[1] = x[1] - eps_e_try  + x[3]*dfdsigy
                res[2] = x[2] - eps_e_trxy + x[3]*dfdsigxy
                res[3] = self.PlasticityModel.f(sigma_current[0], sigma_current[1], sigma_current[2], lamda_current)
                ...
                jac[3,0] = dfdsigx*dsig1depse1 + dfdsigy*dsig2depse1
                jac[3,1] = dfdsigx*dsig1depse2 + dfdsigy*dsig2depse2
                jac[3,2] = dfdsigxy*mu
                jac[3,3] = dfdlamda
