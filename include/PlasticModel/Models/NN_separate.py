...
class Multiply(nn.Module):
  ...
  def forward(self, inp_list):
    device = inp_list[0].device
    result = torch.ones(inp_list[0].size()).to(device)
    for x in inp_list:
      result *= x
    return result
...
class NeuralNetwork_f(nn.Module):
  def __init__(self, INPUT_dim, OUTPUT_dim, WIDTH):
    ...
    self.relu3 = nn.ELU()
    self.fc4 = nn.Linear(WIDTH, WIDTH, bias=True)
    self.relu4 = nn.ELU()
    self.fc5 = nn.Linear(WIDTH, OUTPUT_dim, bias=True)
